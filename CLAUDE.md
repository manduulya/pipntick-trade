...
- **`trading_accounts`** (`accounts.ts`, exported as `tradingAccounts`) — `id` (uuid), `userId` (FK → `users.id`, cascade delete), `name` (default `"Default"`), `broker` (nullable), `currency` (default `"USD"`), `startingBalance` (`numeric(18,2)`, default `"0"`), `isDefault` (boolean), `brokerUtcOffsetMinutes` (`integer`, nullable — the broker platform's server-clock UTC offset, e.g. `120` for UTC+2; see the time-handling note below), `createdAt`, `updatedAt`. Represents a "trading account" per `docs/app-flow.md`'s Add/Create Trading Account step — a user can have several, but Journal/Performance currently only ever operate on the default one.
...
**Trade-time timezone handling**: `entryTime`/`exitTime` store the **broker-server wall-clock digits** the user typed (or that screenshot OCR read), labeled `Z` but *not* a real UTC instant — `apps/web/src/lib/trade-utils.ts` explains why, and everything downstream (Journal/calendar/dashboard display, calendar day-bucketing, `formatDateTime`) reads them back verbatim with no conversion. The trade form's date fields are labeled with the account's offset (`Entry Date & Time (UTC+2)`) via `formatUtcOffsetLabel`, and only three things convert the wall-clock to a real instant using `trading_accounts.brokerUtcOffsetMinutes` (`brokerWallClockToUtc` in `time-format.ts`): the "not in the future" check, the "not before account start" check, and `detectSession`'s UTC-hour London/NY/Tokyo lookup. When the offset is null/0 the fields are plain UTC. The API's `POST`/`PATCH /api/trades` future-time guard can't see the account offset, so it uses a deliberately wide (~14 h) tolerance and leaves the precise check to the client. Fixed offset only — it does not track broker DST; proper IANA-zone support is deferred. Trades logged before this (typed into the old `(UTC)` field) may have a stored `session` that's off by the offset until re-edited.
